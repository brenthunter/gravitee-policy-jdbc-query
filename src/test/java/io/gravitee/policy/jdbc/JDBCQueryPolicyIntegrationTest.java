...
class JDBCQueryPolicyIntegrationTest {
    ...
    static class TestPreparer extends AbstractPolicyTest<JDBCQueryPolicy, JDBCQueryPolicyConfiguration> {
    ...
    @Nested
    @GatewayTest
    @DeployApi({ "/apis/jdbc-api.json", "/apis/jdbc-api-with-EL.json", "/apis/jdbc-api-invalid-connection-string.json" })
    class OnRequest extends TestPreparer {

        @Test
        @DisplayName("Test 1 - With the prepared configuration in jdbc-api.json, should successfully get results")
        void test1_should_successfully_get_results(HttpClient httpClient) {
            ...
            wiremock.verify(1, getRequestedFor(urlPathEqualTo("/endpoint")).withHeader(X_JDBC_HEADER, equalTo(CREATED)));
        }
    ...
    void test2_should_successfully_with_myuserid_provided(
      ...
      wiremock.verify(
        ...
        void test3_should_fail_with_sql_syntax_error_when_no_myUserId_provided(HttpClient httpClient) {
            wiremock.stubFor(get("/endpoint").willReturn(ok()));
            ...
            wiremock.verify(
                1,
                getRequestedFor(urlPathEqualTo("/endpoint")).withHeader(X_JDBC_HEADER, equalTo(ERROR_PROCESSING_JDBC_SQL_SYNTAX_ERROR))
            );
        }

        @Test
        @DisplayName("Test 4 - Should be successful with myUserId provided (to the SQL statement)")
        void test4_should_successfully_with_myUserId_provided(HttpClient httpClient) {
            wiremock.stubFor(get("/endpoint").willReturn(ok()));
        ...
        @Test
        @DisplayName("Test 5 - Should be successful (but empty) with non-existent myUserId provided (to the SQL statement)")
        void test5_should_successfully_but_empty_with_non_existent_myUserId_provided(HttpClient httpClient) {
            wiremock.stubFor(get("/endpoint").willReturn(ok()));
            ...
            wiremock.verify(1, getRequestedFor(urlPathEqualTo("/endpoint")).withHeader(X_JDBC_HEADER, equalTo(NO_RESULTS)));
        }
        ...
        void test6_should_fail_because_the_supplied_server_name_is_bad_in_the_JDBC_Connection_String(HttpClient httpClient) {
            wiremock.stubFor(get("/endpoint").willReturn(ok()));
            ...
            wiremock.verify(1, getRequestedFor(urlPathEqualTo("/endpoint")).withHeader(X_JDBC_HEADER, containing("SQL Exception")));
        }
    }
}

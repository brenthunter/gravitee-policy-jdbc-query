...
A ```jdbcResponse``` object will be created from the results of your SQL statement and the header `X-JDBC: success` will be added.  Use the ```jdbcResponse``` object to extract desired field values into Context Variables.  

Important:  if no Variables are configured in the Policy, then the full response will be set in the 'jdbcResponse' context attribute.

TODO: For better performance, a separate JDBC Resource should be created so that this Policy can re-use existing JDBC connections (rather than having to create for every request).
